//! This module will contain extensions of some TriviumStream of KreyviumStream
//! objects, when trans ciphering is available to them.

use crate::{TriviumStreamByte, TriviumStreamShortint};
...
/// Triat specifying the interface for trans ciphering a FheUint64 object. Since
/// it is meant to be used with stream ciphers, encryption and decryption are by
/// default the same.
...
fn transcipher_from_fheu8_stream(stream: Vec<FheUint8>, cipher: FheUint64, fhe_server_key: &ServerKey) -> FheUint64 {
...
fn transcipher_from_1_1_stream(
  ...
  let pairs = (0..32)
    ...
    .map(|i| {
      ...
      casting_key.cast(&internal_server_key.unchecked_add(b0, &internal_server_key.unchecked_scalar_mul(b1, 2)))
    })
    .collect::<Vec<_>>();
...
impl TransCiphering for TriviumStreamByte<FheUint8> {
  /// `TriviumStreamByte<FheUint8>`: since a full step outputs 8 bytes, these
  /// bytes are each shifted by a number in [0, 8), and XORed with the input
  /// cipher
  fn trans_encrypt_64(&mut self, cipher: FheUint64) -> FheUint64 {
